...
func (s *Server) initHandlers() {
	...
	r.Post("/api/v1/candidates/upload-url", s.getUploadURL)
...
func (s *Server) createCandidate(w http.ResponseWriter, r *http.Request) {
	...
	if in.Type == 0 {
		in.Type = entity.CandidateTypeTelegram
	}
...
func (s *Server) getUploadURL(w http.ResponseWriter, r *http.Request) {
	var req dto_models.GetUploadURLRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		httpErrorf(w, http.StatusBadRequest, "invalid JSON: %v", err)
		return
	}

	result, err := s.candidateService.GetUploadURL(r.Context(), req.CandidateID, req.VacancyID)
	if err != nil {
		httpErrorf(w, http.StatusInternalServerError, "failed to generate upload URL: %v", err)
		return
	}

	resp := dto_models.GetUploadURLResponse{
		UploadURL: result.UploadURL,
		FileKey:   result.FileKey,
		ExpiresIn: int((time.Minute * 10).Seconds()),
	}
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(resp)
}
