...
const (
	defaultUploadPresignTTL = time.Minute * 10
)
...
func (s *ResumeStorage) GetPresignedUploadURL(ctx context.Context, candidateID int64, vacancyID uuid.UUID) (service_models.PresignedUploadResult, error) {
	objectKey := fmt.Sprintf("%d/%s", candidateID, vacancyID)

	url, err := s.client.PresignedPutObject(ctx, s.bucket, objectKey, defaultUploadPresignTTL)
	if err != nil {
		return service_models.PresignedUploadResult{}, fmt.Errorf("presign put: %w", err)
	}

	return service_models.PresignedUploadResult{
